            ...
            <table class="table text-center bodyTable">
                ...
                <tbody>
                    @foreach ($offices as $office)
                        <tr>
                            ...
                            <td>
                                <div class="text-center">
                                    <a href="{{ route('companies.index', ['building' => $floor->building->id, 'floor' => $floor->id, 'office' => $office->id]) }}"
                                        class="link upt-link btn btn-m">Entreprises</a>
                                </div>
                            </td>
                            <td>
                                <div class="text-center">
                                    <a href="#updateModal{{ $office->id }}" class="link upt-link btn btn-u"
                                        data-bs-toggle="modal">Modifier</a>
                                </div>
                            </td>
                            <td class="pt-2">
                                <form
                                    action="{{ route('offices.destroy', ['building' => $floor->building->id, 'floor' => $floor->id, 'office' => $office->id]) }}"
                        ...
                        <div class="modal fade" id="updateModal{{ $office->id }}" tabindex="-1"
                            ...
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <form
                                            action="{{ route('offices.update', ['building' => $floor->building->id, 'floor' => $floor->id, 'office' => $office->id]) }}"
                                            method="POST">
                                            @csrf
                                            @method('PUT')
                                            <label for="office-name" class="form-label ">Nom du bureau</label>
                                            <input type="text" class="form-control" id="office-name" name="name"
                                                value="{{ $office->name }}" required>
                                            <div>
                                                <label for="is_rented" class="form-label mt-3">Disponible</label>
                                                <div class="form-check">
                                                    <input type="radio" class="form-check-input" id="is_rented"
                                                        name="is_rented" value={{ $office->is_rented }}
                                                        {{ $office->is_rented == 0 ? 'checked' : '' }}>
                                                    <label class="form-check-label" for="is_rented">Oui</label>
                                                </div>
                                                <div class="form-check">
                                                    <input type="radio" class="form-check-input" id="is_rented"
                                                        name="is_rented" value={{ $office->is_rented }}
                                                        {{ $office->is_rented == 1 ? 'checked' : '' }}>
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form id="add-office-form"
                                action="{{ route('offices.store', ['building' => $floor->building->id, 'floor' => $floor->id]) }}"
                                method="POST">
                                @csrf
                                @method('POST')
                                ...
                                <div>
                                    <label for="is_rented" class="form-label mt-3">Disponible</label>
                                    <div class="form-check">
                                        <input type="radio" class="form-check-input" id="is_rented" name="is_rented"
                                            value={{ 0 }} checked>
                                        <label class="form-check-label" for="is_rented">Oui</label>
                                    </div>
                                    <div class="form-check">
